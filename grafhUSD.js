function LiveReports3()
            ...
            let MyCoins=[];
                 for(i=0;i<info.length;i++)
                    if(info[i].toggle1==true)
                        MyCoins.push(info[i].symbol);
            options.title = {
                text:`${MyCoins.join(",").toUpperCase()} to USD`,
                fontFamily: "-apple-system, sans-serif"
            };
        ...
        function UpdateMyReports()
            {
                ...
                let y = MyDataCoins[MyCoinName]["USD"];
                let x = new Date();
        ...
        function GetMyDataToreport(MyCoins1)
            {
                const MyURL = `https://min-api.cryptocompare.com/data/pricemulti?fsyms=;;;MyCoins;;;&tsyms=USD`;
